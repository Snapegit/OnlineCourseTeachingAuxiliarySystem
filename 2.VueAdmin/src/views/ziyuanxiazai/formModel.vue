		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="下载时间" prop="xiazaishijian">
							<el-date-picker
								class="list_date"
								v-model="form.xiazaishijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.xiazaishijian?true:false"
								placeholder="请选择下载时间" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.xiazaishijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='xiazaishijian'){
					form.value.xiazaishijian = row[x];
					disabledForm.value.xiazaishijian = true;
					continue;
				}
				if(x=='yonghuzhanghao'){
