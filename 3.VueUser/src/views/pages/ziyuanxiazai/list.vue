	...
	<div class="app-contain" :style='{"width":"64%","padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"20px","background":"#fff"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('ziyuanxiazai','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="资源名称" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.ziyuanmingcheng}}
							</template>
						</el-table-column>
						<el-table-column label="资源类型" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.ziyuanleixing}}
							</template>
						</el-table-column>
						<el-table-column label="下载时间" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.xiazaishijian}}
							</template>
						</el-table-column>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/ziyuanxiazaiAdd')
	}
	//判断是否从个人中心跳转
	...
	const tableDetailClick = (row) => {
		router.push('ziyuanxiazaiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
