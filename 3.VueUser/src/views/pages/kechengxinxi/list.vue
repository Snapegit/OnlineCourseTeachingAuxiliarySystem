<template>
	<div class="app-contain" :style='{"width":"64%","padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"20px","background":"#fff"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('kechengxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.kechengfengmian && item.kechengfengmian.substr(0,4)=='http'" @click.stop="preViewClick(item.kechengfengmian)">
							<el-image class="data_img" :src="item.kechengfengmian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.kechengfengmian.split(',')[0])">
							<el-image class="data_img" :src="item.kechengfengmian?$config.url + item.kechengfengmian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title">{{item.kechengmingcheng}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/kechengxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.kechengfenlei = categoryList.value[categoryIndex.value]
		}
	...
	const detailClick = (id) => {
		router.push('kechengxinxiDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
