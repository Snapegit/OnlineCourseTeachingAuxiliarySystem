...
<script setup>
	...
	const handleRegister = () => {
		...
		if(registerForm.value.yonghuzhanghao&& (String(registerForm.value.yonghuzhanghao).length<6)){
			context?.$toolUtil.message(`用户账号长度不能小于6`,'error')
			return false
		}
		if(registerForm.value.yonghuzhanghao&& (String(registerForm.value.yonghuzhanghao).length>10)){
			context?.$toolUtil.message(`用户账号长度不能大于10`,'error')
			return false
		}
		...
		if(registerForm.value.yonghumima&& (String(registerForm.value.yonghumima).length<6)){
			context?.$toolUtil.message(`用户密码长度不能小于6`,'error')
			return false
		}
		if(registerForm.value.yonghumima&& (String(registerForm.value.yonghumima).length>10)){
			context?.$toolUtil.message(`用户密码长度不能大于10`,'error')
			return false
		}
...
<style lang="scss" scoped>
	.register_view {
		...
		background: url(http://clfile.zggen.cn/20231017/a8c17adedf994b518b6e377f3eeb5cac.jpg);
		display: flex;
		min-height: 100vh;
