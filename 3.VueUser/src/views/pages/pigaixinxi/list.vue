	...
	<div class="app-contain" :style='{"width":"64%","padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"20px","background":"#fff"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('pigaixinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="批改内容" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								<el-button v-if="scope.row.pigaineirong" type="text" size="small" @click.stop="download(scope.row.pigaineirong)">下载</el-button>
								<el-button v-else disabled type="text" size="small">无</el-button>
							</template>
						</el-table-column>
						<el-table-column label="分数" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.fenshu}}
							</template>
						</el-table-column>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/pigaixinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const tableDetailClick = (row) => {
		router.push('pigaixinxiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
